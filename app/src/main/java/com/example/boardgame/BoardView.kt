...
class BoardView(context: Context?, attrs: AttributeSet?) : View(context, attrs) {
    ...
    private val imageId = setOf(
                                    ...
                                    R.drawable.black_piece_king,
                                    R.drawable.red_piece_king)
    ...
    private fun drawWhilePlaying(canvas : Canvas) {
        ...
        movingPieceBitmap?.let {
            canvas.drawBitmap(it, null, RectF(movingPieceX - shift / 2, movingPieceY - shift / 2, movingPieceX + shift / 2, movingPieceY + shift / 2), paint)
        }
    }


    private fun drawPieceAt(canvas: Canvas, col: Int, row: Int, imageId: Int) {
        canvas.drawBitmap(bitmaps[imageId]!!, null, RectF(originX + col * shift, originY + (7 - row) * shift, originX + (col + 1) * shift, originY + ((7 - row) + 1) * shift), paint)
    }
    ...
    private fun drawSquareAt(canvas: Canvas, col: Int, row: Int, isDark: Boolean) {
        ...
        canvas.drawRect(originX + col * shift, originY + row * shift, originX + (col + 1) * shift, originY + (row + 1) * shift, paint)
        if (!isDark) {
            var darkSquare = DarkSquares(col, row)
